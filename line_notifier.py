...
LINE\_ACCESS\_TOKEN = "rYoHKe/26fKDyrXUkXUqtCSrqgy/9KCqa9HK3DNTAp47IxJHr/mkgON9iSdWVtuoKqKyNa9fAap6XhwYE4QJOzWmoyyJqJ1vmH3N8TM+dIiK4JeFkb0xCSxU2uyWH1dB/WjCekHCNTBX3luQzl7DZgdB04t89/1O/w1cDnyilFU="
USER\_ID = "U83ffcfd731f340eb571d8f050131eae2"
...
def calculate\_distance(lat1, lon1, lat2, lon2):
"""
...
def check\_and\_send\_line\_notification(item\_data, user\_lat=BASE\_LAT, user\_lng=BASE\_LNG):
"""
新規投稿（クマダスやアプリ内投稿）の辞書データを受け取り、5km以内ならLINE送信する。
"""
try:
\# 投稿データの緯度経度を取得
item\_lat = float(item\_data.get("lat", 0))
item\_lng = float(item\_data.get("lng", 0))
...
周辺（{distance\_rounded}km圏内）でクマの情報が投稿されました。

■ 日時: {date\_str}
■ 場所: {location\_str}
■ 距離: 現在地から約 {distance\_rounded}km
■ 状況: {status\_str}
■ 詳細: {detail\_str}
■ 情報元: {source\_str}
